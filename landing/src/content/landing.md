PRODUCT: GUI WORKSPACE MANAGER FOR AI-DRIVEN DEVELOPMENT
STATUS: AVAILABLE
VERSION: 0.1.0
PLATFORM: MACOS
~~~
ABOUT:
Mission control for parallel AI coding. Workbench is a single GUI for every git worktree, every Claude Code agent, and every running branch — all at once. No more juggling terminal windows. No more losing track of which agent is working on what.
Built for engineers running Claude Code at scale. Keep your editor; add one place to see everything.
~~~
INSTALL:
- curl -fsSL https://blaise1030.github.io/install.sh | sh
- [Install options](https://blaise1030.github.io/)
- [Releases](/releases)
~~~
FEATURES:
- Parallel git worktrees, tracked in one sidebar
- Claude Code hook integration — get notified when agents stop or complete tasks
- Per-worktree terminal tabs, git panel, and file explorer
- Line-level diffs, staged/unstaged review without leaving the app
- Keybinding configuration and network settings
- Single binary — no Electron, no runtime dependencies
~~~
HOW IT COMPARES:
- Tower / GitHub Desktop: git GUIs, no worktree-first workflow, no agent awareness
- herdr: lives inside your terminal, not a GUI
- Cursor / VS Code: editors that run one agent at a time
- Workbench: the only GUI built for parallel agents + git worktrees together
~~~
LINKS:
- [Docs](/docs)
- [Source](https://github.com/Blaise1030/workbench-cli)
- [Releases](/releases)
- [Changelog](/changelog)
~~~
~*~ © 2026 WORKBENCH ~*~
